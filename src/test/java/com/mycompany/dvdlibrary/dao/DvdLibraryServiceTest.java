...
public class DvdLibraryServiceTest {
    ...
    public void setUp() {
      ...
      template.execute("insert into dvds(title, rating) values ('it', 'PG')");
      template.execute("insert into dvds(title, rating) values ('it', 'R')");
      template.execute("insert into dvds(title, rating) values ('ET', 'PG')");
    ...
    public void testSearchDvdsByTitle() {
        ...
        List<Dvd> results = service.searchDVDs(criteria);
        System.out.println("KIRAN 1"+results.size());
